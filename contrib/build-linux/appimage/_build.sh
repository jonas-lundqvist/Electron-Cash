...
CFLAGS="-ffile-prefix-map=$BUILD_ROOT=. -fmacro-prefix-map=$BUILD_ROOT=. -g0" "$python" -m pip install --no-deps --no-warn-script-location --no-binary :all: --cache-dir "$CACHEDIR/pip_cache" -r "$CONTRIB/deterministic-build/requirements-pip.txt"
CFLAGS="-ffile-prefix-map=$BUILD_ROOT=. -fmacro-prefix-map=$BUILD_ROOT=. -g0" "$python" -m pip install --no-deps --no-warn-script-location --no-binary :all: --cache-dir "$CACHEDIR/pip_cache" -r "$CONTRIB/deterministic-build/requirements-build-appimage.txt"
CFLAGS="-ffile-prefix-map=$BUILD_ROOT=. -fmacro-prefix-map=$BUILD_ROOT=. -g0" "$python" -m pip install --no-deps --no-warn-script-location --no-binary :all: --cache-dir "$CACHEDIR/pip_cache" -r "$CONTRIB/deterministic-build/requirements.txt"
CFLAGS="-ffile-prefix-map=$BUILD_ROOT=. -fmacro-prefix-map=$BUILD_ROOT=. -g0" "$python" -m pip install --no-deps --no-warn-script-location --no-binary :all: --only-binary PyQt5,PyQt5-Qt5 --cache-dir "$CACHEDIR/pip_cache" -r "$CONTRIB/deterministic-build/requirements-binaries.txt"
